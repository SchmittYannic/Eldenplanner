...
export const initialState: CommentsStateType = commentsAdapter.getInitialState({
    totalComments: 0,
    hasMore: true,
...
// Create the base selector for the comments state
// export const selectCommentsQueryResult = (state: RootState, queryArgs: any) =>
//     commentApiSlice.endpoints.getComments.select(queryArgs)(state);

// const selectCommentsData = createSelector(
//     selectCommentsQueryResult,
//     (commentsResult) => commentsResult.data ?? commentsAdapter.getInitialState()
// );
...
//     (state: RootState) => {
//         return selectCommentsData(state, {
//             lastFetchedTimestamp: state.comments.lastFetchedTimestamp,
//             sort: state.comments.sort,
//             limit: state.comments.limit,
//         })
...
// Memoized selector to get the query parameters
...
// Base selector to get the comments data
const selectCommentsData = createSelector(
    selectCommentsState,
    (commentsState) => ({
        ids: commentsState.ids,
        entities: commentsState.entities
    })
);

// Use the base selector with query parameters
export const selectAllComments = createSelector(
    [selectCommentsData, selectQueryParams],
    (commentsData, queryParams) => {
        // This selector can now use commentsData and queryParams
        return commentsAdapter.getSelectors().selectAll({ ids: commentsData.ids, entities: commentsData.entities });
    }
);
